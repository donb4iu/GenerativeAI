...
class AI():
    def __init__(self):
        ...
        db = chromadb.PersistentClient("RAG/output")
#        self.collection = db.get_or_create_collection("nvidia")
        self.collection = db.get_collection("nvidia")
    ...
    def respond(self, lst_messages, model="phi3", use_knowledge=False):
        ...
        res = ollama.chat(model=model, messages=[{"role":"system", "content":prompt}]+lst_messages, stream=True)

        for r in res:
            chunk = r["message"]["content"]
